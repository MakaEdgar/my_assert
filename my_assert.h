...
#define FORMATTED_STR_IMPL_(text, code_start, code_end) \
    "\033[1;" #code_start "m" text "\033[" #code_end "m"

#define FORMATTED_STR_IMPL(text, code_start, code_end) \
    FORMATTED_STR_IMPL_(text, code_start, code_end)
...
#define MYDEBUG(expr)                                                                             \
    ...
    do {                                                                                          \
        std::ostringstream oss;                                                                   \
        oss << BOLD_STR(LOCATION ": ") << YELLOW_STR("debug: ") << TOSTR(expr) << " = " << (expr) \
            << std::endl;                                                                         \
        std::cerr << oss.str();                                                                   \
    } while (false)
...
#define MYWARNING(expr)                                                                      \
    do {                                                                                     \
        if (!(expr)) {                                                                       \
            std::ostringstream oss;                                                          \
            oss << BOLD_STR(LOCATION ": ") << MAGENTA_STR("warning check failed: ") << #expr \
                << std::endl;                                                                \
            std::cerr << oss.str();                                                          \
        }                                                                                    \
    } while (false)
